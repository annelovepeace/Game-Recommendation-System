...
path_app_stats = 'data/app_stats.json'
path_user_inventory = 'data/user_inventory.txt'

engine = create_engine('mysql+pymysql://:@127.0.0.1/game_recommendation?charset=utf8mb4') # local mysql server
...
with open(path_app_info, 'r') as f:
	...
	for raw_string in lst_raw_string:
		steam_id,app_data = json.loads(raw_string).items()[0]
		if app_data == {}:
...
df_popularity_based_results = pd.Series(dic_steamspy.get('owners').values(),dic_steamspy.get('owners').keys()).sort_values(ascending=False).to_frame()
...
with open(path_user_inventory,'r') as f:
	...
	current_count = 0
	for i in lst_all:
		user_id, user_inventory = json.loads(i).items()[0]
...
df_als_result.index.name = 'steam_appid'
